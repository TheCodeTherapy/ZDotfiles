...
make_caps_hyper() {
  print_info "Replacing CAPS key by Hyper key ..."

  # Update /etc/default/keyboard
  sudo cp "${DOTDIR}"/dotfiles/etc/keyboard_hyper /etc/default/keyboard ||
    handle_error "Failed to copy keyboard_hyper file."
  sudo udevadm trigger --subsystem-match=input --action=change ||
    handle_error "Failed to trigger udevadm."

  # Update GNOME settings using gsettings or dconf
  print_info "Updating GNOME settings to map CAPS to Hyper_L ..."
  if command -v gsettings &>/dev/null; then
    ...
    dconf write /org/gnome/desktop/input-sources/xkb-options "['caps:hyper']" ||
  ...
  # Confirm the update
  if dconf read /org/gnome/desktop/input-sources/xkb-options | grep -q "['caps:hyper']"; then
    print_success "CAPS key successfully remapped to Hyper_L."
  else
